...
function [bins, binaverage, r_max] = intensity_radialAverage_slices(img_data, center, ang_bounds, r_max, nbs)
...
%Filling the bins
% ang_bounds = [0,120,240,360];

zero_mat= zeros(nbs,1);

binindex = cell(length(ang_bounds)-1,1);
bintotal = cell(length(ang_bounds)-1,1);
binaverage = cell(length(ang_bounds)-1,1);

parfor i = 1:length(ang_bounds)-1   
    binindex{i} = zero_mat;
    bintotal{i} = zero_mat;
    binaverage{i} = zero_mat;
    radi = rad(ang>=ang_bounds(i) & ang < ang_bounds(i+1));
    radi_val = rad_val(ang>=ang_bounds(i) & ang < ang_bounds(i+1));
    
for iii = 1:nbs-1
    for iv = 1:length(rad(ang>=ang_bounds(i) & ang < ang_bounds(i+1)))
        if radi(iv)>=bins(iii)
			if radi(iv)<bins((iii + 1))
                binindex{i}(iii) = binindex{i}(iii) + 1;
                bintotal{i}(iii) = bintotal{i}(iii) + radi_val(iv);
            end
        end
    end
end

binaverage{i} = bintotal{i}./binindex{i};
end
